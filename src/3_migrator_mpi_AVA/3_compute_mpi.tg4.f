      SUBROUTINE COMPUTE_TG4(NF, DIMGRUPPOTR, DIMBLOCCOTR,
...
CCC 03/05/2002 MPI-
      ...
      INTEGER*4     W_A, W_B                                  
      REAL*4        TG_WEIGHTS_LIN_B(W_B), TG_WEIGHTS_LIN_A(W_A), TG_WEIGHTS_CUB_B(4,W_B,4)
...
**********Variabili locali**********************************************
      ...
      INTEGER*4     IBB,IBB_P,IAA,IASTARTFITTO,IAA_P,IASTARTFITTOFIRST,BASEB,BASEC,BASEB_LIN,BASE_LIN
      REAL*4        PLIN_A(2),PLIN_B(2),PCUB_B(4)
...
! COORDINATE DEL MID-POINT
           ...
           ICSTART_MAXAPE=(MAX(ICC_MID-APEMAX_C,1.0))
           NCC_REAL=PAN3
           ICEND_MAXAPE=(MIN(ICC_MID+APEMAX_C,NCC_REAL))           
                     ...
                     INDICE = BASE_LIN
                       ...
                       DO 763 J = 1,2
                          DO 764 I = 1,2
                             ...
                             ENDIF
#endif                             
                             IF (FLAG.EQ.0 .AND. ((RUZ_LASCO(J,I).eq.-2.0) .or.(RA_LASCO(J,I).eq.-2.0) 
     &                            .or. (SA_LASCO(J,I).eq.-2.0))) THEN
                             	write(6,*)"incongruenza"
                             	FLAG=1
                             	GOTO 770                            	
                             ENDIF
 764                      CONTINUE
...
! CALCOLO AMPIEZZE E COSENO DIRETTORE Z CON INTERPOLAZIONE LIN+LIN E TEMPI CON CUBICA+LIN
                            DO 780 I=1,2                            
                                ...
                                DO 781 J=1,2
                                    AR_LASCO(I)    = AR_LASCO(I)    + PLIN_B(J)*RA_LASCO(J,I)
                                    ASORG_LASCO(I) = ASORG_LASCO(I) + PLIN_B(J)*SA_LASCO(J,I) 
                                    CZR_LASCO(I)   = CZR_LASCO(I)   + PLIN_B(J)*RUZ_LASCO(J,I)                             
#ifdef AVACODE    
                                    RVX_LASCO2(I)   = RVX_LASCO2(I)   + PLIN_B(J)*RVX_LASCO1(J,I)  
                                    RVY_LASCO2(I)   = RVY_LASCO2(I)   + PLIN_B(J)*RVY_LASCO1(J,I)  
                                    RVZ_LASCO2(I)   = RVZ_LASCO2(I)   + PLIN_B(J)*RVZ_LASCO1(J,I)  
                                    SVX_LASCO2(I)   = SVX_LASCO2(I)   + PLIN_B(J)*SVX_LASCO1(J,I)  
                                    SVY_LASCO2(I)   = SVY_LASCO2(I)   + PLIN_B(J)*SVY_LASCO1(J,I)  
                                    SVZ_LASCO2(I)   = SVZ_LASCO2(I)   + PLIN_B(J)*SVZ_LASCO1(J,I)  
#endif      
 781                            CONTINUE
...
! --- IF(3) ---- controllo se i dati distano dal punto meno di maxdisty e maxdistx ------*
     ...
     $                                 (MAXDISTX.EQ.-1) ) .AND. 
     $                              ( ((ABS(IC-ICC_S) .LE. MAXDISTY ).AND.
     $                                 (ABS(IC-ICC_R) .LE. MAXDISTY)) .OR. 
     $                                 (MAXDISTY.EQ.-1) ) )  THEN
#endif
...
#ifdef AVACODE
                                             RVX_FITTO   = PLIN_A(1)*RVX_LASCO2(1)   + PLIN_A(2)*RVX_LASCO2(2)
                                             RVY_FITTO   = PLIN_A(1)*RVY_LASCO2(1)   + PLIN_A(2)*RVY_LASCO2(2)
                                             RVZ_FITTO   = PLIN_A(1)*RVZ_LASCO2(1)   + PLIN_A(2)*RVZ_LASCO2(2)
                                             SVX_FITTO   = PLIN_A(1)*SVX_LASCO2(1)   + PLIN_A(2)*SVX_LASCO2(2)
                                             SVY_FITTO   = PLIN_A(1)*SVY_LASCO2(1)   + PLIN_A(2)*SVY_LASCO2(2)
                                             SVZ_FITTO   = PLIN_A(1)*SVZ_LASCO2(1)   + PLIN_A(2)*SVZ_LASCO2(2)
#endif                      
